...
class Pass2Nodes {
    static void run(Resource resource, PipelineContext ctx) {
        ...
        prePopulateDecideConditionGroups(ctx);
            ...
            if (obj instanceof org.omg.sysml.lang.sysml.Element) {
                ...
                if (id != null) {
                    ...
                    // 解析常规 Action (根据 typed 决定类型)
                        ...
                        // 跳过 loop body wrapper 及嵌套定义
                        if (handleActionUsageSkipCheck(obj, id, name, ctx))
                            continue;
                        handleActionUsageWithPins(obj, id, name, ctx);
                        if (name.startsWith("checkIf"))
                            lastCheckIfUuid = id;
                    }
    ...
    /** 处理 ReferenceUsage: 赋值操作创建 Assign_ 节点, 非赋值创建占位符 */
    private static void handleReferenceUsage(EObject obj, String id, String name, PipelineContext ctx) {
    ...
    /** 跳过不应创建的 ActionUsage: loop body wrapper / Package 级别空定义 / 嵌套定义 */
    private static boolean handleActionUsageSkipCheck(EObject obj, String id, String name, PipelineContext ctx) {
    ...
    /** 处理 ActionUsage: 创建 CallBehaviorAction/CallOperationAction, 提取 Pin 和继承参数 */
    private static void handleActionUsageWithPins(EObject obj, String id, String name, PipelineContext ctx) {
        MainRunner.uuidToNameMap.put(id, name);
        MainRunner.nameToIdMap.put(name, id);
    ...
    /** 处理 IfActionUsage: 创建 DecisionNode + MergeNode, 提取条件并创建分支节点 */
    private static boolean handleIfActionUsage(EObject obj, String id, String name, PipelineContext ctx,
            String lastCheckIfUuid) {
    ...
    /** 处理 LoopActionUsage: 创建 until 循环结构 (Merge->body->Decision->回边) */
    private static void handleLoopActionUsage(EObject obj, String id, String name, PipelineContext ctx) {
        String mergeId = id + "_loopMerge";
        String decisionId = id + "_loopDecision";
        MergeNode merge = (MergeNode) ctx.activity.createOwnedNode((name != null ? name : "Loop") + "_Merge",
                UMLPackage.Literals.MERGE_NODE);
        DecisionNode decision = (DecisionNode) ctx.activity
                .createOwnedNode((name != null ? name : "Loop") + "_Decision", UMLPackage.Literals.DECISION_NODE);
                    ...
                    for (EObject ore : child.eContents()) {
                        ...
                        if ("ForLoopActionUsage".equals(oreCn) || "WhileLoopActionUsage".equals(oreCn)
                            ...
                            if ("ForLoopActionUsage".equals(oreCn))
                                res = LoopExpander.expandNestedForLoop(ctx.activity, ore, ctx.sysmlBasePath);
                            else if ("WhileLoopActionUsage".equals(oreCn))
                                res = LoopExpander.expandNestedWhileLoop(ctx.activity, ore, ctx.sysmlBasePath);
                            else
                                res = LoopExpander.expandNestedLoopAction(ctx.activity, ore, ctx.sysmlBasePath);
    ...
    /** 处理 ForLoopActionUsage: 创建 for-each 循环结构 (Merge->Body->Decision) */
    private static void handleForLoopActionUsage(EObject obj, String id, String name, PipelineContext ctx) {
        String mergeId = id + "_forLoopMerge";
        String decisionId = id + "_forLoopDecision";
        String loopName = (name != null ? name : "ForLoop");
        MergeNode merge = (MergeNode) ctx.activity.createOwnedNode(loopName + "_Merge", UMLPackage.Literals.MERGE_NODE);
        ...
        System.out.println("[FOR-LOOP] id=" + id.substring(0, Math.min(8, id.length())) + " var=" + loopVarName
                + " range=" + rangeText + " body="
                + (bodyActionId != null ? bodyActionId.substring(0, Math.min(8, bodyActionId.length())) : "null"));
    }
    ...
    /** 处理 ForkNode: 创建 UML ForkNode */
    private static void handleForkNode(EObject obj, String id, String name, PipelineContext ctx) {
    ...
    /** 处理 JoinNode: 创建 UML JoinNode */
    private static void handleJoinNode(EObject obj, String id, String name, PipelineContext ctx) {
    ...
    /** 处理 TerminateActionUsage: 跳过不相关的, 否则创建 terminate 节点 */
    private static void handleTerminateActionUsage(EObject obj, String id, PipelineContext ctx) {
    ...
    /** 处理 MergeNode: 创建 UML MergeNode */
    private static void handleMergeNode(EObject obj, String id, String name, PipelineContext ctx) {
    ...
    /** 处理 DecisionNode: 创建 UML DecisionNode, 提取 decide 条件分组 */
    private static void handleDecisionNode(EObject obj, String id, PipelineContext ctx) {
    ...
    private static void handleTransitionUsage(EObject obj, PipelineContext ctx) {
        ...
        for (EObject mChild : obj.eContents()) {
            ...
            if ("Membership".equals(mcn) && !mcn.contains("EndFeature") && !mcn.contains("Parameter")
                    && !mcn.contains("Owning") && !mcn.contains("Transition") && !mcn.contains("Return")) {
                try {
        ...
        if (transSrcId != null && targetName != null) {
            if (condSuccTgtId != null && !MainRunner.nameToIdMap.containsKey(targetName)) {
                ...
                System.out.println(
                        "[DEBUG] TransitionUsage: registered nameToIdMap[" + targetName + "] = " + condSuccTgtId);
            }
            ctx.decideDeferredEdges.add(new String[]{transSrcId, targetName, guardText});
        }
    }

    private static void prePopulateDecideConditionGroups(PipelineContext ctx) {
        try {
    ...
    /** 处理 FlowUsage: flow from X.port to Y.port -> ObjectFlow */
    private static void handleFlowUsage(EObject obj, String id, PipelineContext ctx) {
    ...
    /** 处理 SuccessionFlowUsage: succession flow from X.pin to Y.pin */
    private static void handleSuccessionFlowUsage(EObject obj, String id, PipelineContext ctx) {
        String srcActionName = null, srcPinName = null;
        String tgtActionName = null, tgtPinName = null;
        int endCount = 0;
        for (EObject fmChild : obj.eContents()) {
        ...
        ctx.processedStructuralIds.add(id);
        System.out.println("[DEBUG] SuccessionFlowUsage: " + srcActionName + "." + srcPinName + " -> " + tgtActionName
                + "." + tgtPinName);
    }
    ...
    private static void handleStructuralElements(EObject obj, String id, String name, String className,
        ...
        // 跳过已在其他分支处理的类型
        if (className.contains("Action") || className.contains("Control") || className.contains("ForkNode")
                || className.contains("JoinNode") || className.contains("MergeNode")
                || className.contains("DecisionNode") || className.contains("Transition")
                || className.contains("Terminate") || className.contains("If") || className.contains("Loop")
                || className.contains("Accept") || className.contains("Send") || className.contains("Assign")
                || className.contains("Perform") || className.contains("Exhibit") || className.contains("Include")
                || className.contains("Satisfy") || className.contains("Assert") || className.contains("Expression")
                || className.contains("Feature") || className.contains("Membership") || className.contains("Import")
                || className.contains("Expose") || className.contains("Succession") || className.contains("Comment")
                || className.contains("Doc") || className.contains("Namespace") || className.contains("Package")
                || className.contains("Dependency") || className.contains("Alias") || className.contains("Element")
                || className.contains("Operator") || className.contains("Literal") || className.contains("Invocation")
