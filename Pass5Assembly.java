...
class Pass5Assembly {
    static void run(Resource resource, PipelineContext ctx, String xmiPath) throws Exception {
        ...
        if (!ctx.isCalcModel) {
                    ...
                    if (deepCn.contains("BindingConnector")) {
                        ...
                        if (endFeatureIds.size() >= 2) {
                            ...
                            if (end1 != null && end2 != null) {
                                ...
                                System.out.println("[BIND PASS] " + end1[1] + " <-> " + end2[1]
            ...
            System.out.println("[BIND PASS] Total " + ctx.bindConnectorBindings.size() + " bind connectors, featureIdToPinInfo has " + ctx.featureIdToPinInfo.size() + " entries, processed " + processedBindConnectorIds.size() + " unique connectors");
        }

        // ===================================================================
        // DEFERRED FLOW RESOLUTION: Resolve FlowUsage bindings now that all actions are in nameToIdMap
        // ===================================================================
        for (String[] dflow : ctx.deferredFlows) {
            ...
            if (srcId != null && tgtId != null) {
                ...
                System.out.println("[FLOW] " + srcActionName + "." + srcPinName + " \u2192 " + tgtActionName + "." + tgtPinName);
            } else {
                System.out.println("[FLOW WARN] Unresolved: " + srcActionName + "." + srcPinName + " \u2192 " + tgtActionName + "." + tgtPinName + " (srcId=" + srcId + ", tgtId=" + tgtId + ")");
            }
        }

        // ===================================================================
        // DEFERRED SUCCESSION FLOW RESOLUTION: SuccessionFlowUsage \u2192 ObjectFlow (pinBindings)
        // ControlFlow \u8fb9\u5df2\u5728 PASS 4 \u4e4b\u524d\u6dfb\u52a0\u5230 logicalEdges
        // ===================================================================
        for (String[] dsflow : ctx.deferredSuccessionFlows) {
            ...
            if (srcId != null && tgtId != null) {
                ...
                ctx.pinBindings.add(new String[]{tgtId, tgtPinName, srcId, srcPinName});
                System.out.println("[SUCCESSION FLOW] " + srcActionName + "." + srcPinName + " \u2192 " + tgtActionName + "." + tgtPinName + " (ObjectFlow)");
            } else {
                System.out.println("[SUCCESSION FLOW WARN] Unresolved: " + srcActionName + "." + srcPinName + " \u2192 " + tgtActionName + "." + tgtPinName + " (srcId=" + srcId + ", tgtId=" + tgtId + ")");
            }
        }

        // ===================================================================
        // DATA FLOW PASS: \u521b\u5efa ObjectFlow \u8fde\u63a5 in/out Pin \u4e4b\u95f4\u7684\u6570\u636e\u6d41
        // ===================================================================
        if (!ctx.isCalcModel && ctx.activity != null) {
            ...
            for (String[] bind : ctx.bindConnectorBindings) {
                ...
                if (pin1 != null && pin2 != null) {
                    ...
                    System.out.println("[BIND FLOW WARN] Pin not found: " + pinName1 + "(" + (pin1 != null) + "), " + pinName2 + "(" + (pin2 != null) + ")");
                }
            }
            objFlowCount += bindFlowCount;
            int totalInPins = 0, totalOutPins = 0;
            for (Map<String, InputPin> m : ctx.actionInputPins.values()) totalInPins += m.size();
            for (Map<String, OutputPin> m : ctx.actionOutputPins.values()) totalOutPins += m.size();
            System.out.println("[DATA FLOW PASS] " + objFlowCount + " ObjectFlows (" + bindFlowCount + " bind), "
                + totalInPins + " InputPins, " + totalOutPins + " OutputPins, "
                + ctx.pinBindings.size() + " bindings, " + ctx.bindConnectorBindings.size() + " bindConnectors");
        }

        // ===================================================================
        // PASS 5: \u63a7\u5236\u6d41\u88c5\u914d\u4e0e\u8f93\u51fa (\u4ec5\u975e calc \u6a21\u578b)
        // ===================================================================
        if (!ctx.isCalcModel) {
            ...
            for (MainRunner.EdgeData edge : ctx.finalRoutedEdges) {
                ...
                ActivityNode tgtNode = ctx.dynamicMerges.containsKey(edge.target) ? ctx.dynamicMerges.get(edge.target) : MainRunner.umlNodes.get(edge.target);

                if (srcNode != null && tgtNode != null && !edge.source.equals(edge.target)) {
                    String key = srcNode.hashCode() + " -> " + tgtNode.hashCode();
                    // \u6709 guard \u7684\u8fb9\u59cb\u7ec8\u521b\u5efa (\u4e0d\u53c2\u4e0e\u53bb\u91cd), \u65e0 guard \u7684\u8fb9\u624d\u53bb\u91cd
        ...
        // ===== UML \u6a21\u578b\u9a8c\u8bc1 =====
        System.out.println("\n========== UML \u6a21\u578b\u9a8c\u8bc1\u62a5\u544a ==========");
        System.out.println("[\u9a8c\u8bc1] \u6d3b\u52a8\u56fe: " + (ctx.activity != null ? ctx.activity.getName() : "null"));
        System.out.println("[\u9a8c\u8bc1] UML \u8282\u70b9\u6570\u91cf: " + MainRunner.umlNodes.size());
        System.out.println("[\u9a8c\u8bc1] \u903b\u8f91\u8fb9\u6570\u91cf: " + (ctx.isCalcModel ? ctx.activity.getEdges().size() : ctx.finalRoutedEdges.size()));
        System.out.println("[\u9a8c\u8bc1] \u53d8\u91cf\u6570\u91cf: " + (ctx.activity != null ? ctx.activity.getVariables().size() : 0));
        
        // \u68c0\u67e5\u662f\u5426\u6240\u6709 ActionUsage \u90fd\u6709\u5bf9\u5e94\u7684 UML \u8282\u70b9
        int actionCount = 0;
        int nodeCount = 0;
        ...
        System.out.println("[\u9a8c\u8bc1] \u521b\u5efa\u7684 ActivityNode \u6570\u91cf (\u4e0d\u542b\u63a7\u5236\u8282\u70b9): " + nodeCount);
        System.out.println("[\u9a8c\u8bc1] \u521b\u5efa\u7684\u63a7\u5236\u6d41\u6570\u91cf: " + ctx.activity.getEdges().size());
        ...
        if (!MainRunner.whileLoopCondText.isEmpty()) {
            ...
            for (String loopId : MainRunner.whileLoopCondText.keySet()) {
                System.out.println("  - Loop " + loopId + ": condition=" + MainRunner.whileLoopCondText.get(loopId) + ", isUntil=" + MainRunner.whileLoopIsUntil.getOrDefault(loopId, false));
            }
        }
        ...
        System.out.println("[COMPLETED] UML \u6a21\u578b\u751f\u6210\u5b8c\u6bd5\u3002\u9690\u5f0f Fork\u3001\u5faa\u73af\u7ed3\u6784\u4e0e Guard \u6761\u4ef6\u5df2\u6ce8\u5165\u3002");
    }
}
