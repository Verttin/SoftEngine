...
class Pass5Assembly {
    static void run(Resource resource, PipelineContext ctx, String xmiPath) throws Exception {
        if (!ctx.isCalcModel) {
            resolveBindingConnectors(resource, ctx);
        }
        resolveDeferredFlows(ctx);
        assembleControlFlows(ctx);
        saveUmlModel(ctx, xmiPath);
        System.out.println("[COMPLETED] UML 模型生成完毕。隐式 Fork、循环结构与 Guard 条件已注入。");
    }

    // ===================================================================
    // BIND CONNECTOR PASS: 深度搜索 BindingConnectorAsUsage
    // ===================================================================
    private static void resolveBindingConnectors(Resource resource, PipelineContext ctx) {
        ...
        System.out.println("[BIND PASS] Total " + ctx.bindConnectorBindings.size()
                + " bind connectors, featureIdToPinInfo has " + ctx.featureIdToPinInfo.size() + " entries, processed "
                + processedBindConnectorIds.size() + " unique connectors");
    }

    // ===================================================================
    // DEFERRED FLOW RESOLUTION: FlowUsage + SuccessionFlowUsage
    // ===================================================================
    private static void resolveDeferredFlows(PipelineContext ctx) {
        for (String[] dflow : ctx.deferredFlows) {
            ...
            if (srcId != null && tgtId != null) {
                ...
                System.out.println("[FLOW WARN] Unresolved: " + srcActionName + "." + srcPinName + " → " + tgtActionName
                        + "." + tgtPinName + " (srcId=" + srcId + ", tgtId=" + tgtId + ")");
            }
        }
    ...
    // ===================================================================
    // DATA FLOW + CONTROL FLOW: ObjectFlow 创建 + ControlFlow 装配
    // ===================================================================
    private static void assembleControlFlows(PipelineContext ctx) {
    ...
    // ===================================================================
    // SAVE UML MODEL + VALIDATION REPORT
    // ===================================================================
    private static void saveUmlModel(PipelineContext ctx, String xmiPath) throws Exception {
        ...
        System.out.println(
                "[验证] 逻辑边数量: " + (ctx.isCalcModel ? ctx.activity.getEdges().size() : ctx.finalRoutedEdges.size()));
